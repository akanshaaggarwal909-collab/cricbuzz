...
-- Q1: All Indian players with role, batting & bowling style
...
JOIN bowling_performance bo ON p.player_id = bo.player_id AND b.match_id = bo.match_id
JOIN matches m ON b.match_id = m.match_id
WHERE p.playing_role = 'All-rounder'
GROUP BY p.player_id, m.match_format
HAVING SUM(b.runs_scored) > 1000 AND SUM(bo.wickets_taken) > 50;

-- Q10: Last 20 completed matches with result details
SELECT m.match_desc, t1.team_name AS team1, t2.team_name AS team2,
...
-- Q11: Player performance across formats (>=2 formats played)
SELECT p.full_name,
...
-- Q12: Home vs away performance per team
SELECT t.team_name,
...
-- Q13: Partnerships (consecutive batting positions) >= 100 runs
...
-- Q14: Bowling economy by venue (min 3 matches at venue, >=4 overs/match)
...
HAVING COUNT(DISTINCT bo.match_id) >= 3;
...
SELECT p.full_name,
       ...
       SUM(CASE WHEN m.winner_id = b.team_id THEN 1 ELSE 0 END) AS close_matches_won
FROM batting_performance b
JOIN players p ON b.player_id = p.player_id
JOIN matches m ON b.match_id = m.match_id
...
-- Q16: Yearly batting trend since 2020 (min 5 matches/year)
...
-- Q17: Toss impact on match outcome, by decision
SELECT m.toss_decision,
       COUNT(*) AS total_matches,
       SUM(CASE WHEN m.toss_winner_id = m.winner_id THEN 1 ELSE 0 END) AS toss_winner_also_match_winner,
       ROUND(100.0 * SUM(CASE WHEN m.toss_winner_id = m.winner_id THEN 1 ELSE 0 END) / COUNT(*), 2) AS win_pct
FROM matches m
GROUP BY m.toss_decision;

-- Q18: Most economical bowlers, limited-overs (min 10 matches, >=2 overs/match avg)
SELECT p.full_name,
       ROUND(SUM(bo.runs_conceded) * 1.0 / SUM(bo.overs_bowled), 2) AS economy_rate,
       SUM(bo.wickets_taken) AS total_wickets
FROM bowling_performance bo
JOIN players p ON bo.player_id = p.player_id
JOIN matches m ON bo.match_id = m.match_id
...
GROUP BY p.player_id
HAVING COUNT(DISTINCT bo.match_id) >= 10
   AND AVG(bo.overs_bowled) >= 2
...
-- Q20: Matches & batting average per format (min 20 total matches)
SELECT p.full_name,
       ...
       ROUND(AVG(CASE WHEN m.match_format = 'T20I' THEN b.runs_scored END), 2) AS t20_avg
FROM batting_performance b
JOIN players p ON b.player_id = p.player_id
JOIN matches m ON b.match_id = m.match_id
GROUP BY p.player_id
...
-- Q21: Weighted performance ranking (batting + bowling + fielding points)
SELECT p.full_name, m.match_format,
       ROUND(
         (COALESCE(SUM(b.runs_scored), 0) * 0.01) +
         (COALESCE(AVG(b.runs_scored), 0) * 0.5) +
         (COALESCE(AVG(b.strike_rate), 0) * 0.3) +
         (COALESCE(SUM(bo.wickets_taken), 0) * 2) +
         ((50 - COALESCE(AVG(bo.runs_conceded * 1.0 / NULLIF(bo.wickets_taken,0)), 50)) * 0.5) +
         ((6 - COALESCE(AVG(bo.economy_rate), 6)) * 2) +
         (COALESCE(SUM(f.catches), 0) * 3) +
         (COALESCE(SUM(f.stumpings), 0) * 5)
       , 2) AS total_points
FROM players p
LEFT JOIN batting_performance b ON p.player_id = b.player_id
LEFT JOIN bowling_performance bo ON p.player_id = bo.player_id AND b.match_id = bo.match_id
LEFT JOIN fielding_performance f ON p.player_id = f.player_id AND b.match_id = f.match_id
LEFT JOIN matches m ON b.match_id = m.match_id
GROUP BY p.player_id, m.match_format
ORDER BY m.match_format, total_points DESC;

-- Q22: Head-to-head analysis (teams with >=5 matches in last 3 years)
SELECT t1.team_name AS team_a, t2.team_name AS team_b,
       COUNT(*) AS total_matches,
       SUM(CASE WHEN m.winner_id = m.team1_id THEN 1 ELSE 0 END) AS team_a_wins,
       SUM(CASE WHEN m.winner_id = m.team2_id THEN 1 ELSE 0 END) AS team_b_wins,
       ROUND(AVG(CASE WHEN m.winner_id = m.team1_id THEN m.victory_margin END), 2) AS avg_margin_team_a_win,
       ROUND(AVG(CASE WHEN m.winner_id = m.team2_id THEN m.victory_margin END), 2) AS avg_margin_team_b_win
FROM matches m
JOIN teams t1 ON m.team1_id = t1.team_id
JOIN teams t2 ON m.team2_id = t2.team_id
...
-- Q23: Recent player form (last 10 innings) categorization
...
SELECT p1.full_name AS batter1, p2.full_name AS batter2,
       ...
       ROUND(100.0 * SUM(CASE WHEN (b1.runs_scored + b2.runs_scored) > 50 THEN 1 ELSE 0 END) / COUNT(*), 2) AS success_rate
FROM batting_performance b1
JOIN batting_performance b2
     ON b1.match_id = b2.match_id
    AND b1.innings_no = b2.innings_no
...
-- Q25: Quarterly performance trend & career trajectory (>=6 quarters, >=3 matches/quarter)
WITH quarterly AS (
    SELECT b.player_id,
...
SELECT p.full_name, t.quarter, t.avg_runs, t.avg_sr,
       CASE
         WHEN t.prev_avg_runs IS NULL THEN 'Baseline'
...
WHERE t.player_id IN (
    SELECT player_id FROM quarterly GROUP BY player_id HAVING COUNT(*) >= 6
)
